...
def main():
    ...
    # 200개 버전: category는 의류 종류(outer/top/...)라 사전가정 일치율 대신
    # 군집 분포와 (의류종류 × 군집) 교차표를 출력해 해석을 돕는다.
    print("\n군집 분포:", df["cluster_label"].value_counts().to_dict())
    print("\n의류종류 × 군집 교차표:")
    print(pd.crosstab(df["category"], df["cluster_label"]).to_string())
